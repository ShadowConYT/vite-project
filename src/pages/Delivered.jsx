...
const Delivered = () => {
  ...
  // Filter shipments to include only those with ETA in the past
  const deliveredShipments = shipments.filter((shipment) => {
    const etaDate = new Date(shipment.estimated_arrival_date);
    const currentDate = new Date();
    return etaDate < currentDate; // Include only shipments with ETA in the past
  });
  ...
  const rows = !loading
    ? deliveredShipments.map((shipment, index) => {
        ...
        const ltDaysDifference = Math.abs(
          Math.ceil((estimatedArrivalDate - updatedInvoiceDate) / (1000 * 60 * 60 * 24))
        );
  ...
  return (
    <React.Fragment>
      <div className='text-xl font-extrabold mx-5 '>
        <h1>Delivered HBLs</h1>
      </div>
